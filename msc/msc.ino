...
void setup() {
  ...
  usb_msc.setCapacity(1000*64*4,512);//24ブロックは予備
...
int32_t msc_read(uint32_t lba, void *buffer, uint32_t bufsize){
  ...
  int page = bad_block_replace(lba/4);
...
*/
  ...
  memcpy(buffer, &(jisc_ssd_block_buffer[(jisc_ssd_page_size * in_page) + (512*(lba%4))]), bufsize);
  return bufsize;
}
...
*/
  ...
  memcpy(&(jisc_ssd_block_buffer[(jisc_ssd_page_size * in_page) + (512*(lba%4))]), buffer, bufsize);
  haveBufferData = true;
  return bufsize;
}
